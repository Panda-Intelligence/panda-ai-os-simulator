...
#define M5PAPERS3_SD_CS_GPIO 47
...
static void esp32s3_machine_init(MachineState* machine) {
   ...
   */
  {
    ...
    if (mofei_sim_board_is_lilygo_t5s3_pro() || mofei_sim_board_is_m5papers3()) {
      ...
      if (mofei_sim_board_is_lilygo_t5s3_pro()) {
        ...
        /* PaperS3 has SPI2 SD on CS47, but no radio target. */
        esp32s3_machine_attach_lilygo_spi_sd(ss->mofei_spi2, sd_blk, 0);
        fprintf(stderr, "[PAPERS3-SPI] SD media attached cs=%d\n", M5PAPERS3_SD_CS_GPIO);
      }
    } else {
  ...
  /* Board GPIO wiring — connect GPIO pins to peripheral control lines. */
  {
    ...
    if (mofei_sim_board_is_lilygo_t5s3_pro()) {
      ...
      qdev_connect_gpio_out_named(gpio_dev, "gpio-out", M5PAPERS3_SD_CS_GPIO,
                                  qdev_get_gpio_in_named(ss->mofei_spi2, "chip-select", 0));
    } else {
