...
try {
  ...
  assert.equal(await page.locator(".panel-hanging-ear").count(), 1);
  assert.equal(await page.locator('.panel-shell[data-physical-size="121.5×67.7×7.7mm"]').count(), 1);
  ...
  assert.equal(await page.locator(".panel-physical-key").count(), 4);
  assert.equal(await page.locator('.panel-physical-key[title="RST"]').count(), 1);
  assert.equal(await page.locator(".panel-front-home-ring").count(), 1);
  assert.equal(await page.locator('.panel-shell[data-physical-size="129×69×11mm"]').count(), 1);
  report.lilygoFirmware = await firmware.inputValue();
  const physicalKey = page.locator(".panel-physical-key:not(.panel-physical-key--reset)").first();
